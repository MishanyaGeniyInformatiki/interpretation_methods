...
import numpy as np
import torch
from images.utils.vanilla_gradient import VanillaGradient
...
class GradCam(VanillaGradient):
    ...
    def get_mask(self, image_tensor, target_class=None, resize=True):
        ...
        if resize:
            cam = np.array(Image.fromarray(cam).resize((image_tensor.shape[2], image_tensor.shape[3]), Image.Resampling.LANCZOS))
        self.gradient = None
        return cam